...
{
    NODE *cur1 = list1 ? list1->p_head : nullptr;
    NODE *cur2 = list2 ? list2->p_head : nullptr;
    while (cur1 && cur2)
    {
...
{
    // Your code here //
    List *reversedList = createList(nullptr);
    if (L->p_head == nullptr ) {
        return reversedList;
    }

    if (L->p_head->p_next == nullptr) {
        addHead(reversedList,L->p_head->key);
        return reversedList;
    }
    ...
    reversedList = createList(head);
    for (int i = 1; i < n; i++) addHead(reversedList,a[i]);
